...
def exchange_money(budget: float, exchange_rate: float, whole_number: bool = False) -> float:
    ...
    return budget // exchange_rate if whole_number else budget / exchange_rate
...
def get_number_of_bills(amount: int, denomination: int) -> int:
    """
...
def exchangeable_value(budget: float, exchange_rate: float, spread: int, denomination: int) -> int:
    ...
    # Total currency you can get after fees
    exchanged_amount = budget / actual_rate

    # Number of whole bills you can take
    num_bills = int(exchanged_amount // denomination)

    # Maximum exchangeable value
