...
def singleton_ingredients(dishes: list, intersection: Final) -> list:
    ...
    # Map each ingredient to the set of dish indices it appears in
    for idx, dish in enumerate(dishes):
        for ingredient in dish:
            if ingredient not in ingredient_to_dishes:
                ingredient_to_dishes[ingredient] = set()
            ingredient_to_dishes[ingredient].add(idx)

    # Select ingredients that appear in exactly one dish and not in intersection
    singletons = {ingredient for ingredient, dish_indices in ingredient_to_dishes.items()
                  if len(dish_indices) == 1 and ingredient not in intersection}
